...
export async function proposePackage(state: OrderState, preferences?: string): Promise<PackageProposal> {
  ...
  const system = `You are a senior event-planning specialist. Given an event, headcount and budget, you assemble ONE attractive, well-rounded package that fits the budget. Prefer popular items, cover the key categories (venue is chosen separately, so focus on catering, photo/video, music, decor, cake and a couple of delightful extras), and respect the budget — per_guest items multiply by guests, per_graduate by honorees, flat are one-off. Return ONLY JSON: {"itemIds": string[], "note": string}. Use ONLY ids from the catalog. The note is one short, warm sentence explaining the choice.`;
